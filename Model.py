...
class TA_Encoder(nn.Module):
    def __init__(self, dim, depth, channels, channel_rate=1, mlp_dim=256, dropout=0.):
        ...
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                PreNorm(dim, Temporal_Attention(channels=channels, channel_rate=channel_rate)),
                PreNorm(dim, FeedForward(dim[-1], mlp_dim, dropout=dropout))
            ]))
...
class AITST(nn.Module):
    def __init__(self, input_size, patch_size, channels, num_classes, model_dim,
        ...
        self.SA_Encoder = SA_Encoder(dim=input_size, depth=8, channels=channels, channel_rate=channel_rate,
                                     dropout=0.1)
        self.TA_Encoder = TA_Encoder(dim=input_size, depth=8, channels=channels, channel_rate=channel_rate,
                                     dropout=0.1)
